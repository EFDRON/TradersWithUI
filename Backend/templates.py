...
note="""
You have access to real-time and historical market data capabilities.
Use REAL-TIME PRICE CAPABILITIES when you need the current market value of an asset.
- This includes latest executed trade price information.
- Use it to estimate the most recent trading price for decision-making.

Use QUOTE CAPABILITIES when you need bid/ask information.
- This provides market spread, liquidity context, and near-instant pricing signals.
- Prefer quotes when modeling order entry/exit decisions or spread-aware logic.

Use BAR DATA CAPABILITIES when you need OHLCV market structure.
- Use this for trend analysis, technical indicators, volatility estimation, and strategy signals.
- Supports multiple timeframes (intraday to long-term).

Use HISTORICAL TRADE CAPABILITIES when you need tick-level execution history.
- Use this for microstructure analysis, volume behavior, and precise trade sequencing.

Use MARKET SCREENER CAPABILITIES when you need broad market context.
- Includes movers, most active symbols, and market-wide signals.

GENERAL RULES:
- Always prefer the most granular data available for decision-making (trades → quotes → bars).
- Use real-time capabilities for execution decisions and delayed/historical capabilities only for analysis.
- Be aware that some data feeds may be partial market representations depending on subscription level.
"""
